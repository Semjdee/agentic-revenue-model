...
export async function GET(req: NextRequest) {
  ...
  const rows = await db.select().from(schema.conversations).where(eq(schema.conversations.tenantId, auth.tenantId)).orderBy(desc(schema.conversations.lastMessageAt)).limit(100);
