...
**Not yet pushed to the production Neon database or deployed** — this entry documents work verified against the local dev database only. Schema-before-code discipline (established in 9g–9i) means the production `db:push` has to happen before this code is committed/pushed, and this session does not have the production `DATABASE_URL` available (unlike a prior session, it isn't present in `.env`, and Netlify/Neon web endpoints aren't reachable from this sandbox to look it up) — flagged to the user rather than skipped or guessed.
