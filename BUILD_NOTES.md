...
## 9i. Commercialization restructure P0 — FREE/PRO/PREMIUM + real credit top-ups (2026-08-21)

Source: `Master Product Architecture Update.pdf` (42 pages), explicitly framed as a non-breaking commercial/UX layer over the existing platform, not a rebuild — "before changing anything, inspect the existing implementation... if an existing proven system already solves part of the requirement, extend it rather than replacing it." A full plan (`EnterPlanMode`/`ExitPlanMode`, user-approved) preceded this work; see that plan's ground-truth table for what already satisfied the doc (one-time Free grant, failed-run zero-charge, the reserve/max-drawdown policy from §9g, per-agent cost + forecasting, `CreditPricingService`-equivalent in `pricing.ts`) versus real gaps (3-tier plans, entitlements, a real payment processor, hiding raw internals from tenants).

**Payment processor: Flutterwave** (user's choice — Africa-native, native UGX/Mobile Money, fits this platform's Uganda-based tenant base) — mobile money only this pass (MTN/Airtel), not card (card requires client-side field-level encryption via Flutterwave's own JS SDK, a separately-scoped frontend integration). Built against Flutterwave's **current** Orchestrator Direct Charge API (`POST /orchestration/direct-charges`, `GET /charges/{id}`, `flutterwave-signature` HMAC-SHA256 webhook header) — verified live against `developer.flutterwave.com` during this work since it differs materially from the older v3 `/v3/payments` API a training-data recollection would produce. **Not yet live-tested against a real Flutterwave sandbox account** (none exists in this environment) — the first real top-up attempt against a live sandbox key is this connector's actual verification, same as any third-party integration built without live credentials.

- **3-tier plans** — `CREDIT_PLANS` (`schema.ts`) changed from binary `FREE/PAID` to `FREE/PRO/PREMIUM`; the only 4 call sites reading the old value updated in the same change. `modules/billing/plans.ts`'s `PLAN_MONTHLY_CREDITS` is an explicitly-labelled **provisional bootstrap** (formula-derived from the already-real `$0.008/credit` price, not guessed) since the doc's own instruction ("use actual usage data") has no real production volume to compute from yet — `scripts/calculate-plan-credits.ts` computes real P50/P90/P95 cost-per-run/conversation and the doc's requested top-up-package economics table once that data exists; run it periodically and update `plans.ts` from its output. A tenant's plan is set by Platform Admin (`PATCH /api/platform/tenants/[id]/plan` — new, since nothing previously could ever move a tenant off the hardcoded `FREE` default) until real recurring subscription billing exists.
- **`modules/entitlements/service.ts`** — new `EntitlementService`, `hasFeature(tenantId, featureKey)`, sitting above existing RBAC (`hasPermission()`) per the doc's own layered hierarchy (§9), not replacing it. Infrastructure only this pass — no currently-open feature (attribution, influencer intelligence, advertising) is actually gated yet, deliberately, to avoid a same-cycle breaking change to features tenants can already use; full gating is the next increment.
- **Real top-up flow** — `modules/billing/topup.ts` (`initiateTopup`/`fulfillTopup`) + `integrations/payments/flutterwave.ts` (real/DEMO-MOCK connector, same discipline as every other connector in this codebase) + new `credit_purchase_intents` table (bridges a Flutterwave charge back to "which tenant, which package," and is the idempotency guard against a webhook firing more than once for the same payment). `fulfillTopup()` **never trusts the webhook payload's claimed status** — always re-verifies server-side against Flutterwave's own API before calling the unchanged `grantCredits()`. New public webhook `POST /api/public/webhooks/flutterwave`.
- **Hiding raw internals from tenants (§29-31)** — the old tenant-reachable `/api/internal/ai/runs` (model names, token counts, $ cost, raw stop reasons) is gone; that data moved to Platform Admin only (`GET /api/platform/tenants/[id]/ai-runs`, `requirePlatformSession()`-gated). Tenants get a new translated `GET /api/internal/billing/ai-activity` (business-readable: "Customer Conversation — Amara · Completed · 3 credits used"), replacing the Settings "AI Runs" tab with "AI Activity." The Credits & Usage tab's ledger table dropped its `model` column (API payload unchanged — `computeAgentCostBreakdown` still needs it server-side — this was a UI-only removal).
- **Platform Admin AI economics (§40-41)** — `modules/platform/ai-economics.ts`, extending `/platform/analytics`: P50/P90/P95/P99 run cost, cost/lead/qualified-lead/sale (same join pattern `computeCampaignPerformance()` in `advertising/analyst.ts` already uses, applied platform-wide), credits sold/revenue/realized gross margin (genuinely 0 until the first real Flutterwave purchase, not invented), provider failures/loop stops/timeouts aggregated cross-tenant, highest-cost individual runs, first-purchase conversion / repeat-purchase rate.
- **Live Conversation Protection wording (§33)** — `UsageForecastCard` now leads with a simple "ON" badge and one-liner; the exact reserve-threshold/daily-cap numbers moved behind a collapsed "Advanced" disclosure instead of always-visible. No backend change — `reserve-policy.ts` (§9g) untouched.

**Two real bugs caught by this work's own verification, fixed before shipping:**
1. The mock Flutterwave connector's redirect URL was built by naive string concatenation (`` `${redirectUrl}?mock_charge_id=...` ``) — broke when the caller's `redirectUrl` already carried its own query string (`?topup=complete`), producing a malformed `?a=1?b=2` URL. Fixed using the `URL`/`searchParams` API. Caught live via an HTTP-level test against the real dev server, not just a code read.
2. An initial test of the plan-change → entitlement propagation checked the wrong tenant (picked whichever tenant a cross-tenant listing happened to order first, not the one the test's own session belonged to) and appeared to fail — re-run against the correct tenant confirmed the real behavior is correct: a Platform Admin plan change propagates to that tenant's own session on the very next request, since `getEntitlements()`/`resolveMonthlyAllotment()` query `credit_balances` fresh every time rather than trusting a cached JWT claim (neither session type carries plan, by design).

**Verified live**, all against the real database and, for the HTTP-level checks, the real dev server (not just the underlying functions): 3-tier `resolveMonthlyAllotment()` math for all three plans; `EntitlementService.hasFeature()` correctly gates per plan (FREE: 0 features, PRO: 8, PREMIUM: 8 + 3 Premium-only, confirmed `revenue_goal_agent` false on PRO / true on Premium); the reserve policy's existing `checkCreditsForTrigger()` regression-free after the plan-model change; a full top-up cycle end to end (initiate → intent row created → webhook-equivalent fulfill → real `grantCredits()` call → balance increases by exactly the package's credits → **a second fulfillment attempt on the same charge grants nothing and leaves the balance unchanged**, confirming the idempotency guard); the `$20` package's economics compute to exactly 70% margin (matching `TARGET_GROSS_MARGIN`); `/api/internal/billing/packages`, `/ai-activity`, `/credits`, `/forecast`, and `/topup/initiate` all confirmed serving correct, appropriately-shaped data through the real HTTP routing layer (not just direct function calls); a Platform Admin plan change on one specific tenant confirmed propagating to that exact tenant's own session (`monthlyAllotment` jumped from 1000 to the PRO bootstrap 4375 on the very next request) while correctly leaving other tenants untouched; `GET /api/platform/tenants/[id]/ai-runs` confirmed reachable with a real platform-staff session and confirmed **blocked (401)** with a tenant session, proving the isolation actually holds. `npm run build`, `npm run demo-journey`, and `npm run demo-journey-phase2` all clean throughout.

**Explicitly not done in P0** (next increment, per the approved plan): gating any currently-open feature (attribution, influencer intelligence, advertising diagnostics) behind PRO/PREMIUM; Intelligence Levels (`L0_DATA`...`L4_PRESCRIPTIVE`); the "See Why · Pro" progressive-discovery UI components; extending `CREDIT_TRANSACTION_TYPES` with `INITIAL_GRANT`/`PLAN_ALLOCATION`/`REFUND`/`PROMOTIONAL`.

## 9j. Industry templates, team seats & subscription terms (2026-08-21)

Source: `Industry Team Subscription Architecture.pdf` (30 pages), the next layer over 9i — same non-breaking, inspect-and-reuse framing, plus its own closing principle: Industry Template / Subscription Plan / Human Seats / AI Credits stay four independently-configurable concepts, never merged. A full plan (`EnterPlanMode`/`ExitPlanMode`, user-approved) preceded this work.

**Two scoping calls, stated in the plan rather than guessed at mid-build:**
1. **Per-industry pipeline stages stay out of scope.** The doc's worked examples (Solar's "SITE ASSESSMENT," Real Estate's "PROPERTY MATCH," Travel's "ITINERARY") each want custom stage names beyond the platform's existing global 7-value `LEAD_STAGES` enum — but that enum is threaded by literal string through the Kanban board, both AI providers' tool-call generation, follow-up eligibility, cost/qualified-lead filters, and the dashboard funnel. Templates in this pass configure the agent (qualification questions/sales rules/restricted topics/escalation conditions/tone) and suggest KB doc titles + product categories — all things that already flow through the existing generic `agents`/`knowledge_documents`/`products` objects — and deliberately leave `LEAD_STAGES` untouched. True configurable-per-tenant pipelines are flagged as a distinct future initiative, not bundled in here.
2. **Subscription-term purchases reuse the existing one-time-Flutterwave-charge pattern**, not new recurring-billing infra — same shape as how tenant plan is already set manually today. A term purchase charges the discounted total up front; `renewalAmountUsd`/`termEndsAt` are computed and stored for visibility, never auto-charged.

- **Part A — Industry templates**: new `industry_templates` table (key/label/tone/qualification questions/sales rules/restricted topics/escalation conditions/KB-title suggestions/product-category suggestions), data-driven so adding an industry is a row insert. Seeded 3 fully-detailed templates (Solar, Real Estate, Travel — `scripts/seed-industry-templates.ts`, idempotent). `tenants.industry` moved from free text to a real validated `INDUSTRY_CATEGORIES` list (`modules/onboarding/industries.ts`, 13 + Other) — existing free-text values (e.g. a legacy "Solar energy" string) won't match a dropdown option and show as unselected until re-picked; no data is lost. `guided-setup.ts`'s `mapAnswersToAgentConfig()` gained an optional `industryTemplate` param that UNIONS template arrays into the answer-derived ones (dedup, template first) rather than replacing — omitting it reproduces prior behavior byte-for-byte, so every existing caller is unaffected. New `GET /api/internal/industry-templates` (tenant list) and Platform-Admin `GET/POST/PATCH /api/platform/industry-templates` (new "Industry Templates" page in the Platform Admin nav) so templates are editable without a redeploy. Applying a template records `tenants.appliedIndustryTemplateKey` (informational, feeds Part E's adoption analytics only).
- **Part B — Team seats**: `PLAN_INCLUDED_SEATS` (FREE 3 / PRO 7 / PREMIUM 15) + `ADDITIONAL_SEAT_PRICE_USD` ($5) in `plans.ts`. `users.status` (`INVITED`/`ACTIVE`/`SUSPENDED`/`DEACTIVATED`, default `ACTIVE`) added alongside the pre-existing `active` boolean, which nothing here touches — every self-signup/phone/OAuth path keeps producing an immediately-usable `ACTIVE` account; only `POST /api/internal/team` creates a user as `INVITED`, flipping to `ACTIVE` on that user's first successful login (`api/internal/auth/login`). `modules/team/seats.ts`'s `checkSeatAvailability()` counts `ACTIVE + INVITED` as billable (a pending invite reserves capacity); the invite route blocks a FREE tenant at/over its included count with an upgrade-prompt 402, and allows-but-flags `overIncludedSeats` on PRO/PREMIUM (actual extra-seat billing flows through a Part C subscription purchase, no separate seat-invoice system). `modules/team/performance.ts`'s `computeTeamPerformance()` is a new real aggregate over the pre-existing `assignedUserId`/`owner` columns (leads assigned/contacted, opportunities, won revenue, conversion %, avg deal size, avg sales cycle, tasks) — deliberately has no "avg response time" metric, since that would need per-message timestamps this pass doesn't touch. `/team` page gained a seat-usage indicator and a "Performance" tab.
- **Part C — Subscription pricing + term purchase**: `modules/billing/subscription-pricing.ts`'s `computeSubscriptionQuote()` is pure calculation (`TERM_DISCOUNT_PCT`: 6mo 5% / 12mo 10% / 24mo 20%, the doc's own schedule) — AI credits never appear in this calculation at all, enforcing the four-concepts rule structurally rather than by convention. Real purchase reuses the exact `topup.ts` idempotent-intent pattern against a new `subscription_purchase_intents` table and the same `getPaymentConnector()`; on fulfillment (`modules/billing/subscription-purchase.ts`) it sets `creditBalances.plan` and writes a `subscriptions` row. The shared `POST /api/public/webhooks/flutterwave` route now looks a `chargeId` up in `credit_purchase_intents` first, falling back to `subscription_purchase_intents` — one endpoint, two fulfillers. New Settings "Plans & Billing" tab with a live term/seat quote (`GET /api/internal/billing/subscription/quote`) feeding `POST /api/internal/billing/subscription/initiate`, same mobile-money flow as Add Credits.
- **Part D — Onboarding flow**: industry template picker folded into the existing `AGENT_SETUP` step (no new step for it — pre-fills the guided Q&A, fully editable after); new optional `TEAM_INVITE` step inserted between `HEALTH_CHECK` and `GO_LIVE` (additive `ONBOARDING_STEPS` enum value — harmless for tenants already past that point), wrapping the existing team-invite mechanism with no new invite backend, explicitly skippable via `POST /api/internal/onboarding/team-invite/continue` per `ONBOARDING_SPEC.md`'s own "offer contextually, don't force" note. `KNOWLEDGE_IMPORT` (which runs *before* `AGENT_SETUP` in step order) shows a KB-suggestion checklist keyed off the tenant's already-chosen `industry` field rather than the AGENT_SETUP-specific template pick, avoiding a step-ordering dependency — a checklist of suggested doc titles only, never auto-generated content.
- **Part E — Platform Admin analytics**: `modules/platform/growth-analytics.ts` — template adoption (tenant count + paid-plan conversion per template key, including "started from scratch"), seat expansion by plan (billable seats, extra seats), subscription-term preference (purchase count, avg discount, revenue — real `subscriptions` rows only, honestly all-zero until the first real term purchase). Wired into the existing `/platform/analytics` page as a new section.

**Verified live** via an HTTP-level script against the real dev server (22/22 checks passing, script deleted after — same discipline as 9i): tenant login → industry-templates list has the 3 seeded templates → guided-agent preview correctly unions a SOLAR-template qualification question into the generated config → seat availability correctly reports FREE/included=3 → an invite within the cap succeeds, the next one is correctly blocked with 402 → the newly-invited teammate logs in with their temp password and their status flips `INVITED`→`ACTIVE` → team performance endpoint responds → `computeSubscriptionQuote()` for PRO/12-month/10 seats matches the hand-computed $540 (10% discount, 3 extra seats @ $5) exactly → a real subscription purchase completes end to end through the shared mock-Flutterwave webhook route and the tenant's plan actually becomes PRO on the next request → Platform Admin can list/create/deactivate industry templates → the analytics route serves `templateAdoption`/`seatGrowth`/`termDistribution` including the one real test purchase. All verification rows (test teammates, test platform-staff account, test industry template, the test subscription/plan change) were cleaned up afterward, restoring the RayGrid demo tenant to its prior FREE-plan state. `npx tsc --noEmit` and `eslint` both clean across every new/changed file.

**Explicitly not done in this pass** (flagged as future work, per the plan's own P1/P2): true configurable per-tenant/per-industry pipeline stages (the scoping call above); industry templates beyond the initial 3 (same mechanism, just more seed rows — or use the new Platform Admin page); auto-renewal or dunning for an expired subscription term; extra-seat proration mid-term.

**Update:** the user supplied the production `DATABASE_URL` directly. Schema pushed to production Neon and verified there (all 3 new tables, both new columns, zero stray legacy plan values); 3 industry templates seeded in production; code committed and pushed to `main`, auto-deployed by Netlify, and spot-checked live (`/api/internal/industry-templates`, `/api/internal/billing/subscription/quote`, `/api/platform/industry-templates` all reachable and correctly auth-gated on both sites). A subsequent local `npm run build` (once the machine's disk was freed) also completed clean, exit 0, no warnings.

## 9k. Tenant-facing credit/token numbers removed entirely (2026-08-21)

User request: no tenant-facing interface — Owner, Sales, any role/department, no exceptions — should show an actual credit or token number (balance, ledger amount, daily rate, projection, per-agent breakdown). Only a coarse status tied to the reserve policy's own 20% threshold (§9g's "Special AI Notes") should be visible. This is a real tightening beyond §9i's original scope, which only hid *dollars/tokens/model detail* and still showed tenants their exact **credit** count — the new instruction extends that same discipline to credit numbers too.

- New `modules/billing/reserve-policy.ts` export: `summarizeCapacityStatus(balance)` → `{ status: "healthy"|"protected"|"exhausted", percentRemainingBand }`. `percentRemainingBand` is rounded to the nearest 5% (a band, not the real percentage) so it can't be used to back-calculate the actual balance; `status` is literally this file's own `RESERVE_PCT` (20%) threshold — `protected` is the tenant-facing answer to "what happens at 20%: non-essential AI usage pauses, live customer conversations never do."
- **`/api/internal/billing/credits`**: no longer returns `balance` or `pricing` at all — just `plan`, `status`, `percentRemainingBand`, and a `recent` ledger list with `direction: "added"|"used"` replacing the numeric `credits`/`balanceAfter` fields entirely (scrubbed at the API layer, not just hidden in the UI, so it can't be recovered from the network tab either).
- **`/api/internal/billing/forecast`**: collapsed to exactly `{ status, percentRemainingBand }` — dropped `currentBalance`, `monthlyAllotment`, `reserveThreshold`, `avgDailyCredits7d/30d`, `projectedMonthlyCredits`, `daysOfRunway`, and the per-agent numeric breakdown. `modules/billing/forecast.ts`'s richer `predictTenantUsage()` is untouched (still used nowhere else) — kept for a possible future Platform Admin per-tenant drill-down, just no longer surfaced to the tenant route.
- **`/api/internal/billing/ai-activity`**: each row and the summary no longer carry `credits`/`totalCreditsUsed` — a run shows only whether it was charged at all (`isFailure`), never how much.
- **Settings → Credits & Usage**: `CreditsTab` shows plan badge + activity log (type + direction dot, no amount) — no balance line at all. `UsageForecastCard` ("Live Conversation Protection") shows one badge: `~NN%` (the rounded band) plus the existing reserve-policy copy, replacing the old exact-count display, the "Advanced details" toggle, and the per-agent numeric list entirely. `AIActivityTab`'s "Credits Used" stat chip and per-row credit badge are gone — replaced with "Not charged"/"Completed".
- `AddCreditsDialog` (package price/credit-quantity for a *new purchase*) and `PlansBillingTab` (subscription price/discount) were deliberately left as-is — those are purchase-catalog numbers a tenant needs to make a buying decision, not usage/balance data, and are a different category from what this request targeted.

**Verified live** via an HTTP script against the real dev server (11/11 checks, script deleted after): confirmed by exact key-inspection that `/credits`'s response has no `balance` or `pricing` field and no ledger row carries `credits`/`balanceAfter`; confirmed `/forecast`'s response is *exactly* `{status, percentRemainingBand}` (no extra keys); confirmed `/ai-activity` rows and summary carry no credit fields; confirmed the raw JSON text of the credits response contains no literal `"balance"` substring anywhere. `tsc --noEmit` and `eslint` both clean. No schema change — nothing to push to production for this fix.

## 9l. UI/UX Modernization — first slice (2026-08-22)

Source: "Revenue Agent UI/UX Modernization.pdf" — a 37-item frontend spec (typography, design tokens, dashboard hierarchy, trend indicators, AI Revenue Brief, Needs Attention, pipeline value, funnel, channel/traffic reporting, sidebar IA, tables/drawers, empty/loading states, component library, accessibility, responsive, dark-mode-ready tokens), with its own numbered implementation priority (items 1-17) and a literal target dashboard layout diagram (Section 35). Explicitly scoped to frontend: "Do not rebuild stable backend functionality purely for this redesign... preserving the existing application behaviour."

**Ground-truth audit first, matching this session's discipline for the last two PDFs**: much of what the doc assumed was missing already existed. Geist was already the intended font, `globals.css` already had semantic dark-mode-aware tokens (background/surface/foreground/text-*/gridline), `.card` was already 14px radius / subtle border / restrained shadow, and `Button` already had all 4 variants (primary/secondary/ghost/danger) called for in Section 26. The real gap was narrower than the doc's framing suggested, but live browser verification (see below) still caught one thing static reading missed entirely.

**Real bug found and fixed, typography (doc Section 1)**: `getComputedStyle(document.body).fontFamily` on the actual running app returned "Times New Roman" despite Geist being loaded via `next/font/local` and `tailwind.config.ts`'s `fontFamily.sans` pointing at its CSS variable. Nothing was ever actually applying that variable as a `font-family` declaration (Tailwind's Preflight does not do it automatically here, and no `font-sans` utility was in use anywhere). Every browser was silently falling back to its default serif, genuinely confirming the doc's opening complaint, just not for the reason a static grep for serif font names would ever surface. Fixed with an explicit `font-family` declaration on `body` in `globals.css`; reconfirmed via the same computed-style check post-fix.

**Implemented this pass** (covers the doc's own priority items 1 through 12):
- Section 11 analytics consistency, the doc's own "higher priority than cosmetic work" item: real bug where the dashboard's Lead Funnel and Channel/Traffic Source Performance queried leads/conversations with no date-range filter at all, while the KPI cards above them were range-scoped. The exact "conversations=6 at the top, funnel shows 0" class of bug the doc describes. Fixed: funnel and channel/source performance now share the identical [from,to] window as the KPIs; the funnel's top row literally reuses the same conversations count already computed for the KPI card rather than re-deriving it from a different table, so the two numbers can't drift apart by construction. Verified live: channel-performance rows sum exactly to the top KPIs (47+11+3=61 conversations, UGX 630M+60M+0=690M revenue) on real seeded data.
- Section 6/35 KPI hierarchy: `KpiCard` gained a `size="primary"` variant; Revenue and Open Pipeline (new) render large per the doc's target layout, the rest as secondary.
- Section 7 trend indicators: new `components/ui/trend-indicator.tsx` plus `lib/format.ts`'s `percentChange()`. The dashboard route now computes every KPI for the same-length previous period (`previousRange()`) and returns null (never a fabricated 0%/infinity%) when there's no meaningful prior value, per the doc's own explicit rule.
- Section 8 AI Revenue Brief: new `modules/dashboard/brief.ts`. Deliberately a deterministic templated summary over real computed numbers, not a live LLM call. This platform's own established "platform calculates, AI interprets" split (execution-gateway.ts) already treats a real model call as something with real cost/complexity/audit-trail, and turning already-correct numbers into a sentence has no ambiguity an LLM would resolve better. Every sentence is built from a real number; a bullet is omitted (never filler text) when its condition doesn't hold.
- Section 9 Needs Attention: extended from 4 to 6 real items (added Quotations Pending value and Underperforming Campaigns, reusing `computeCampaignPerformance()` rather than duplicating campaign-ROAS logic); zero-value rows are hidden rather than shown as noise, with an "all clear" message when every count is 0.
- Section 10 compact currency: new shared `lib/format.ts`'s `formatCompactCurrency()` (UGX 500K / 1.2M / 18.7M / 1.4B), safe for both server (brief text) and client.
- Section 12 lead funnel: now shows top-relative conversion percentage per stage (matching the doc's own 72/120=60% worked example) plus an overall Conversation-to-Sale percentage headline stat.
- Section 17/18 channel and traffic-source performance: enriched from a raw conversation count to conversations/qualified/sales/revenue/conversion per channel and per UTM source (in-memory join over conversations to leads to opportunities.firstConversationId to sales, same idiom as `platform/ai-economics.ts` rather than a multi-table SQL join). Channel Performance is now a real table; Traffic Source became "Revenue by Source."
- Section 19 pipeline value: Open Pipeline and Weighted Pipeline added as new KPIs. Deliberately a live snapshot ("as of now"), not scoped to the selected date range, same reasoning as Needs Attention: "how much is in the pipeline right now" isn't "how much entered in the last 7 days." Weighted uses an AI-proposed per-stage win-probability table (`STAGE_WIN_PROBABILITY`), flagged in-code as provisional pending real win-rate data, same status as `reserve-policy.ts`'s thresholds. "Underperforming campaign" reuses this platform's existing ROAS >= 3 "good" bar (`KpiCard`'s tone logic) as the stand-in for a per-campaign target-ROAS field, which doesn't exist in the schema yet.
- Section 13/14 sidebar IA: `shell.tsx`'s flat 16-item nav regrouped into labelled sections (Overview/Customers/AI/Growth/Analytics/Platform/Admin) per the doc's exact grouping; Channels moved under Platform next to Integrations per the doc's own suggestion (route unchanged).
- Section 23/24 empty and loading states: new `components/ui/empty-state.tsx` and `components/ui/skeleton.tsx`, applied to the dashboard's chart/revenue-by-source/channel-performance cards (skeleton on initial load, empty-state copy instead of a bare "no data").

**Verified live** against a real dev server: `tsc --noEmit` and `eslint` clean; a 28-check HTTP script (deleted after) confirmed the funnel-top/KPI-conversations equality, monotonically non-increasing funnel stages, and response shape across all 3 range presets; then logged into the actual browser and cross-checked the rendered page's real numbers by hand (channel rows sum exactly to top KPIs) and caught plus fixed the font bug above via `getComputedStyle`, which no amount of source-reading would have surfaced.

**Explicitly not done this pass** (the doc's own lower-priority items 13-17, deferred rather than rushed):
- Section 15/16 top navigation bar and "Ask Revenue AI" launcher: the doc frames Section 16 as "remove the cartoon robot," but no floating assistant of any kind exists in this app today, so this isn't a redesign, it's a new feature: a real conversational business-intelligence AI surface. Building that is genuinely new AI backend work (new prompts, a new AIExecutionGateway-routed flow, credit cost) that goes beyond "refactor frontend, preserve existing behaviour," and putting a decorative button that goes nowhere on the page would violate this codebase's own "never fake a completed integration" rule. Deferred as a real feature request, not silently dropped.
- Section 20/21 sitewide table modernization and detail drawers beyond the dashboard's own new Channel Performance table: leads/contacts/campaigns/etc. tables untouched.
- Section 28/29 onboarding checklist widget and widget-install UX polish: separate, sizeable feature work; the existing onboarding wizard (9j and earlier) covers the underlying flow already.
- Section 30/34 full mobile responsive audit and accessibility audit across every page: the dashboard itself already reflows via existing responsive breakpoints, but no dedicated pass was done.
- Section 33 remaining component library breadth: Select/Checkbox/Switch/Avatar/Tooltip/Dropdown/Pagination/DateRangePicker/Toast/ChartContainer not built; only the pieces this pass actually needed (TrendIndicator, Skeleton, EmptyState) were added, extending `KpiCard` in place rather than duplicating it.
- Section 37 final cross-page visual consistency audit: only performed for the dashboard/sidebar, not every other page.

No schema change in this pass; nothing to push to production. This is a pure frontend/API-response-shape change, committed and pushed directly.

## 9m. Real WhatsApp integration — first slice (2026-08-22)

Source: LAUNCH_CHECKLIST.md, item 1. Prior state: inbound webhook shape was already Meta-compatible by design (`api/public/webhooks/whatsapp/route.ts`'s own header comment says so), but there was no real outbound send anywhere in the codebase, no webhook signature verification, and the connect flow was fully mocked (`whatsapp-mock-connector.ts`).

**Honest scope note up front**: full Meta "Embedded Signup" (the JS-SDK popup letting a business owner pick/create their own WABA inline) needs a Business Login Config ID issued from a live Meta App dashboard, and Meta App Review approval before it works for arbitrary external businesses — neither is something this environment can create or test. Built instead: a manual-credential connect flow (the tenant pastes a permanent System User access token + phone number ID + WABA ID from their own Meta Business Manager) — a legitimate, commonly-used pattern for the WhatsApp Cloud API, not a shortcut standing in for the real thing. Every method still makes a real Graph API call and fails honestly on bad credentials, same "never a hardcoded success" rule the mock connector already documented for itself.

Built:
- `integrations/oauth/whatsapp-sender.ts` — real outbound send via `POST /{phone-number-id}/messages`. Wired into both reply paths that previously only ever wrote to this app's own DB: the AI reply path (`modules/conversations/engine.ts`, both the normal reply and the credits-exhausted handoff message) and the human-agent send route (`api/internal/conversations/[id]/messages/route.ts`) — via one shared `modules/conversations/channel-dispatch.ts` helper so neither path can forget to actually deliver the message. No-op for a widget conversation or a still-mock/unconnected WhatsApp integration — existing behaviour there is unchanged.
- Real webhook signature verification (`X-Hub-Signature-256`, HMAC-SHA256 against `META_APP_SECRET`) and a real `hub.verify_token` check on the GET handshake, both in `api/public/webhooks/whatsapp/route.ts` — previously anyone who learned a tenant's phone_number_id could POST fake customer messages; now they can't without the app secret. Both gate on the relevant env var being set, mirroring the mock-skips-verification pattern used everywhere else (Flutterwave's `verifyWebhookSignature`).
- `integrations/oauth/whatsapp-real-connector.ts` implementing the full `OAuthConnector` interface (getAuthorizationUrl/handleCallback/exchangeAuthorizationCode/testConnection/getGrantedScopes/registerWebhooks/unregisterWebhooks) against real Graph API endpoints. Registered in `registry.ts`, gated on `META_APP_SECRET` — same one-line-swap pattern as every other real-vs-mock provider in this codebase.
- Widened `OAuthConnector`'s `exchangeAuthorizationCode`/`testConnection`/`registerWebhooks`/`unregisterWebhooks` signatures with an optional `extra` payload (additive, mock connectors ignore it) so a connector can carry provider-specific data — WhatsApp's WABA id — through `connect-flow.ts`'s shared plumbing without a parallel storage mechanism.
- `api/internal/integrations/[id]/disconnect/route.ts` — previously only ever flipped local DB status, never told the provider the connection was going away. Now best-effort calls the real connector's `unregisterWebhooks`/`revokeAccess` first (never blocks local disconnect if the remote call fails).
- New `/onboarding/whatsapp-connect` real credential-entry page, reusing the existing generic `OAuthMockConsentForm` shell (widened with optional `helperText`/`submitLabel` props so its copy stops saying "Demo authorization" on the real page).

**Verified without needing a real Meta account** (nothing here required a live Meta App): `tsc`/`eslint` clean. Mock-mode regression — the full existing connect → inbound webhook → AI reply flow still works unchanged (0 regressions). Signature verification — a validly-signed request passes through to the phone-number lookup (proving it isn't blocking legitimate traffic), while an unsigned, wrong-secret, or tampered-body request is rejected with 401 (4 checks against a real HMAC computed with a test secret, not a mock). Verify-token handshake — correct token echoes the challenge, wrong token gets 403. Registry gate — confirmed the authorization URL actually changes from `/onboarding/whatsapp-mock-consent` to `/onboarding/whatsapp-connect` once `META_APP_SECRET` is set, proving the real connector is genuinely active, not just present in the codebase.

**What's NOT verified and genuinely can't be from here**: every actual Graph API call (send a real message, verify a real phone number, register real webhooks, introspect real token scopes) — these need a real Meta App and real tenant credentials, which don't exist in this environment. See LAUNCH_CHECKLIST.md's "WhatsApp — status" section for the exact remaining setup steps (create a free Meta Developer App, provide `META_APP_ID`/`META_APP_SECRET`/a chosen `WHATSAPP_WEBHOOK_VERIFY_TOKEN`, configure the webhook URL in Meta's dashboard, then each tenant connects their own account). No schema change in this pass.
